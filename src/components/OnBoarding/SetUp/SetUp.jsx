...
const SetUp = () => {
  ...
  return (
   ...
   <p className="text-center w-full font-semibold text-dark text-[clamp(1rem,_3vw,_1.25rem)] justify-self-start"> {currentStep === 1 ? "Review Profile" : currentStep === 2 ? "Language" : "Time Zone"}</p>
    </div>
      <div className="flex justify-between w-[47.5rem] mx-auto my-6">
        {steps?.map((step, i) => (
          ...
          >
            <div className="step">
              {i + 1 < currentStep || complete ? <FaCheck className="text-heading font-semibold" /> : i + 1}
            </div>
            <p className="text-dark font-medium text-mediumSubheading">{step}</p>
          </div>
        ))}
      </div>
      ...
      {currentStep === 1 ? <ReviewProfile /> : currentStep === 2 ? <SelectLanguage /> : <TimeZone />}

      <div className="absolute w-full text-smallSubheading text-light bottom-4 text-center">
        <span>www.pro-cs.in | contact@pro-cs.in | ProCS - Digital Board Meeting</span>
      </div>
